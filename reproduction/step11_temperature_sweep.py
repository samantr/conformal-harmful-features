...
# ============================================================
# 2. Create the SAME synthetic dataset as Step 10
# ============================================================
...
# ============================================================
# 5. SAME tiny neural network as Step 10
# ============================================================
...
def evaluate_temperature(T):
    ...
    with torch.no_grad():
        ...
        conformal_probs = torch.softmax(
            conformal_logits.double() / T,
            dim=1
        ).numpy()

        test_probs = torch.softmax(
            test_logits.double() / T,
            dim=1
        ).numpy()
...
for T in [0.5, 1.0, 1.3]:
    ...
    with torch.no_grad():
        probs = torch.softmax(
            test_logits[sample_index].double() / T,
            dim=0
        ).numpy()
...
plt.title(
    "Temperature vs Non-Randomized APS Average Set Size"
)
