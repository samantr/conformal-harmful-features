...
# ==========================================
# 4. EVALUATE ONE TEMPERATURE WITH RAPS
# ==========================================
...
def evaluate_temperature(T):
    ...
    # etc.
    true_ranks = (
        true_ranks_zero_based + 1
    )


    # RAPS penalty
    penalty = lam * torch.clamp(
        true_ranks - k_reg,
        min=0
    )


    # Randomized RAPS score
    calibration_scores = (
        previous_sum
    ...
    # --------------------------------------
    # Rank penalty for every candidate class
    # --------------------------------------

    number_of_classes = test_probs.shape[1]
    ...
    rank_penalties = lam * torch.clamp(
        ranks - k_reg,
        min=0
    )

    rank_penalties = rank_penalties.unsqueeze(0)


    # --------------------------------------
    # Randomized RAPS candidate scores
    # --------------------------------------
    ...
    # Put scores back to original class order
...
print(
    "================ FASHION-MNIST RAPS RESULTS ================"
)

print(
    f"k_reg = {k_reg}, lambda = {lam}"
)
